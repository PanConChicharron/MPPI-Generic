...
#ifdef __CUDA_ARCH__
#define RACER_ABS(x) fabsf(x)
#else
#define RACER_ABS(x) std::fabs(x)
#endif

  float t = RACER_ABS(dot2(tx, ty, a0x, a0y));
  float r = a_hl + RACER_ABS(dot2(b0x, b0y, a0x, a0y)) * b_hl + RACER_ABS(dot2(b1x, b1y, a0x, a0y)) * b_hw;
  if (t > r)
  {
    return false;
  }

  t = RACER_ABS(dot2(tx, ty, a1x, a1y));
  r = a_hw + RACER_ABS(dot2(b0x, b0y, a1x, a1y)) * b_hl + RACER_ABS(dot2(b1x, b1y, a1x, a1y)) * b_hw;
  if (t > r)
  {
    return false;
  }

  t = RACER_ABS(dot2(tx, ty, b0x, b0y));
  r = b_hl + RACER_ABS(dot2(a0x, a0y, b0x, b0y)) * a_hl + RACER_ABS(dot2(a1x, a1y, b0x, b0y)) * a_hw;
  if (t > r)
  {
    return false;
  }

  t = RACER_ABS(dot2(tx, ty, b1x, b1y));
  r = b_hw + RACER_ABS(dot2(a0x, a0y, b1x, b1y)) * a_hl + RACER_ABS(dot2(a1x, a1y, b1x, b1y)) * a_hw;
  if (t > r)
  {
    return false;
  }
