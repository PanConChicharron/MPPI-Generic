                                                        ...
                                                        const float steer, const float u_steer, float& K, float& K_dot)
{
  ...
  const float tau = params.steer_time_constant;
  const float inv_L = 1.0F / L;
  const float tan_steer = tanf(steer);
  const float sec2_steer = 1.0F + tan_steer * tan_steer;
  K = inv_L * tan_steer;
  const float steer_dot = (u_steer - steer) / fmaxf(tau, 1.0E-4F);
...
__host__ __device__ float clampPathTrackingCost(const float cost)
{
  constexpr float kMax = 1e16F;
  if (cost > kMax)
  {
    return kMax;
  }
...
{
  (void)crash_status;
  return clampPathTrackingCost(pathTrackingStateCost(this->params_, y.data(), timestep));
}
                                                                     ...
                                                                     const int timestep, int* crash)
{
  ...
  return clampPathTrackingCost(cost);
}
...
{
  (void)crash_status;
  return clampPathTrackingCost(pathTrackingStateCost(this->params_, y, timestep));
}
