...
void CONTROLLER::copySampledControlFromDevice(bool synchronize)
{
  ...
  random_sample_indices_.resize(num_sampled_trajectories);
  if (perc_sampled_control_trajectories_ > 0.98)
  {
    // if above threshold just do everything
    std::iota(random_sample_indices_.begin(), random_sample_indices_.end(), 0);
  }
  else
  {
    // Create sample list without replacement
    // removes the top 2% since top 1% are complete noise
    random_sample_indices_ = mppi::math::sample_without_replacement(num_sampled_trajectories, getNumRollouts() * 0.98);
  }
  ...
  for (int i = 1; i < num_sampled_trajectories; i++)
  {
    HANDLE_ERROR(cudaMemcpyAsync(this->sampled_outputs_d_ + i * getNumTimesteps() * DYN_T::OUTPUT_DIM,
                                 this->output_d_ + random_sample_indices_[i] * getNumTimesteps() * DYN_T::OUTPUT_DIM,
                                 sizeof(float) * getNumTimesteps() * DYN_T::OUTPUT_DIM, cudaMemcpyDeviceToDevice,
                                 this->vis_stream_));
    HANDLE_ERROR(cudaMemcpyAsync(
        this->sampler_->getVisControlSample(i, 0, 0), this->sampler_->getControlSample(random_sample_indices_[i], 0, 0),
        sizeof(float) * getNumTimesteps() * DYN_T::CONTROL_DIM, cudaMemcpyDeviceToDevice, this->vis_stream_));
  }
  if (synchronize)
  {
                                                       ...
                                                       const Eigen::Ref<const sampled_cost_traj>& costs)
{
  ...
  float min_sample_value = costs[temp_list[min_sample_index]];

  for (int index = 1; index < temp_list.size(); index++)
  {
    if (costs[temp_list[index]] < min_sample_value)
    {
      min_sample_value = costs[temp_list[index]];
...
void CONTROLLER::copyTopControlFromDevice(bool synchronize)
{
  ...
  for (int i = num_top_control_trajectories_; i < getNumRollouts(); i++)
  {
    ...
    {  // Remove the smallest weight in the current list and add the new index
      top_n_sample_indices_[min_sample_index] = i;
      // recalculate min weight in the current list
      std::tie(min_sample_index, min_sample_value) =
          findMinIndexAndValue(top_n_sample_indices_, this->trajectory_costs_);
    }
  }
  ...
  for (int i = 0; i < num_top_control_trajectories_; i++)
  {
    top_n_costs_[i] = trajectory_costs_[top_n_sample_indices_[i]] / getNormalizerCost();
    HANDLE_ERROR(cudaMemcpyAsync(
        this->sampled_outputs_d_ + (start_top_control_traj_index + i) * getNumTimesteps() * DYN_T::OUTPUT_DIM,
        this->output_d_ + top_n_sample_indices_[i] * getNumTimesteps() * DYN_T::OUTPUT_DIM,
    ...
    HANDLE_ERROR(cudaMemcpyAsync(this->sampler_->getVisControlSample(start_top_control_traj_index + i, 0, 0),
                                 this->sampler_->getControlSample(top_n_sample_indices_[i], 0, 0),
